...
On AMD, the process is a little bit more complicated. PCI I/O must be performed using Index / Data ports on the PCI root complex. After writing [`0x03810994`](#references) to index port `0xB8`, a `DWORD` can be read back from port `0xBC` that shows the status of PSB.
